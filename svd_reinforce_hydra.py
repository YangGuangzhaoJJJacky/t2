...
def main(cfg):
    ...
    print("lr",cfg.lr)
    print("kl",cfg.kl_ref_coeff)
    print("interval",cfg.test_interval)
    ...
    print("resuming_from_ckpt",resuming_from_ckpt)
    ...
    if test_only and (not resuming_from_ckpt):
        ...
        log_dir = f'{cfg.output_path}/{task_name}_{policy_name}_{exp_name.replace("/", "_")}'
    ...
    for i in range(num_iters+1):
        print(f"in iters {i}")
        if i == 0:
            print("skip trian in 0 iter only test")
        else:
